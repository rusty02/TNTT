...
{
    public class MienController : ApiControllerBase
    {
        // GET: all Mien in TNTT
        public HttpResponseMessage Get()
        {
            try
            {
                var mienList = new List<MienModel>();
                var list = uow.Miens.GetAll().OrderBy(c => c.Id);
                foreach (var mien in list)
                {
                    mienList.Add(new MienModel()
                    {
                        Id = mien.Id,
                        StartDate = mien.StartDate,
                        EndDate = mien.EndDate,
                        ChaTuyenUy = uow.Contacts.GetById(mien.ChaTuyenUy.HasValue ? mien.ChaTuyenUy.Value : 0),
                        ChuTich = uow.Contacts.GetById(mien.ChuTich.HasValue ? mien.ChuTich.Value:0),
                        PhoQuanTri = uow.Contacts.GetById(mien.PhoQuanTri.HasValue ? mien.PhoQuanTri.Value : 0),
                        PhoNghienHuan = uow.Contacts.GetById(mien.PhoNghienHuan.HasValue ? mien.PhoNghienHuan.Value : 0),
                        ThuKy = mien.ThuKy,
                        ThuQuy = mien.ThuQuy,
                        Website = mien.Website,
                        Status = mien.Status,
                    });
                }
               
                return Request.CreateResponse(HttpStatusCode.OK, mienList);
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, "Error loading Mien list");
            }
